-- name: SuperDisplays
-- description: Show information about sm64excoop\nFeature\n - Speed Show\n - Action Id lab\n - Position\n - Level Date\nbyToad114514\n
-- v1.02
...
local function on_hud_render_behind()
  if menu == true then
    ...
    djui_hud_render_rect(19, 62, 115, 155)
    
    --- @type MarioState
    local m = gMarioStates[0]
    local actext = sActionTable[m.action] or "????????????"
    local posmario = gMarioStates[0].pos
    local level_data = tostring(gNetworkPlayers[0].currLevelNum)
    
    djui_hud_set_font(FONT_HUD)
    djui_hud_set_color(255, 255, 255, 255)
    djui_hud_print_text("Speed " .. tostring(math_floor(m.forwardVel)):gsub("-", "M"), 23, 68, 0.7)

    djui_hud_set_font(FONT_HUD)
    djui_hud_set_color(255, 255, 255, 255)
    djui_hud_print_text("Action",23,80,0.7)
    
    djui_hud_set_font(FONT_NORMAL)
    djui_hud_set_color(255,255,0,255)
    djui_hud_print_text(actext, 23, 89, 0.4)
    
    djui_hud_set_font(FONT_HUD)
    djui_hud_set_color(255,255,255,255)
    djui_hud_print_text("Position",23,101,0.7)
    
    djui_hud_set_font(FONT_NORMAL)
    djui_hud_set_color(255,255,0,255)
    djui_hud_print_text(math.floor(posmario.x) .. " " .. math.floor(posmario.y) .. " "  .. math.floor(posmario.z),23,110,0.4)
    
    djui_hud_set_font(FONT_HUD)
    djui_hud_set_color(255,255,255,255)
    djui_hud_print_text("Level " .. level_data,23,121,0.6)
  end
end
...
local function on_level_init()
    close_menu()
    on_hud_render_behind()
end

local function command_display(msg)
    if msg == "on" then
      menu = true
      return true
    elseif msg == "off" then
...
hook_chat_command("debug", "[on|off] Enable or Disable SuperDisplay",command_display)
